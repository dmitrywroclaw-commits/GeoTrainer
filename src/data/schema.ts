...
const entry = z.discriminatedUnion('kind', [
  base.extend({ kind: z.literal('flag'), countryId: id, variantType: z.enum(['national', 'state', 'civil', 'state_ensign', 'civil_ensign', 'other_official']), symbols: z.array(symbol).min(1) }),
  ...
  base.extend({ kind: z.literal('landmark'), countryIds: z.array(id).min(1), landmarkType: z.string().min(1), whyNotableRu: z.string().min(1) }),
]);

export const librarySchema = z.object({
  countries: z.array(z.object({ id, nameRu: z.string().min(1), region: z.string().min(1) })),
